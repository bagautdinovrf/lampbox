...
PrettySlider::PrettySlider( Qt::Orientation orientation, SliderMode mode,
                                    QWidget *parent, uint max )
    : Slider( orientation, parent, max )
    , m_mode( mode )
{
    if( m_mode == Pretty)
      {
...
void
PrettySlider::mousePressEvent( QMouseEvent *e )
{
...
QSize
PrettySlider::minimumSizeHint() const
{
    return sizeHint();
}

QSize
PrettySlider::sizeHint() const
{
    constPolish();
...
VolumeSlider::VolumeSlider( QWidget *parent, uint max )
    : Slider( Qt::Horizontal, parent, max )
    , m_animCount( 0 )
    , m_animTimer( new QTimer( this ) )
    , m_pixmapInset( QPixmap( "://img/volumeslider-inset.png" ))
{
    setFocusPolicy( Qt::NoFocus );
...
void
VolumeSlider::generateGradient()
{
   // volumeslider-gradient.png
    const QImage mask( "://img/volumeslider-gradient.png" );
...
void
VolumeSlider::slotAnimTimer() //SLOT
{
    if ( m_animEnter ) {
        m_animCount++;
        update();
...
void
VolumeSlider::paintEvent( QPaintEvent * )
{
    QPainter p(this);
...
void
VolumeSlider::leaveEvent( QEvent* )
{
